...
**Engine ↔ theme boundary (since `backend-core 0.1.62`):** the customer-facing **storefront + account theme** — all `app/(storefront)` pages except `checkout`, all `app/(account)` pages, and their presentation components (`components/{product,layout,storefront,marketing}/**` + the cart-page components) — is **per-client and excluded from core sync**. Edit it freely in a client for that brand's copy *and layout* — it never drifts or gets overwritten. The shared **engine** (`lib`/`stores`/`hooks`/`actions`/`types`, `components/ui`, checkout + payment, admin & ops consoles, auth, root infra) stays core; changes there go through the template + release train. **Engine code must never import a theme file** — enforced by `frontend/scripts/check-theme-boundary.mjs` in `ci:reliability-gates`. Truly bespoke pages still go in `app/(client)/**` / `components/client/**`.
