...
7. `docs/THIRD_PARTY_INTEGRATIONS_SETUP_AND_KEY_MANAGEMENT_GUIDE.md` — provider keys, rotation, and incident drill policy.
   - `docs/META_WHATSAPP_SETUP_GUIDE.md` — full per-client WhatsApp Business (Cloud API) onboarding: Business Suite, System User token, app, phone number, webhook, templates (7 utility incl. `return_request_update` + `otp_verify` auth), App Review/publish, and where each key goes (**Ops → Config, not `.env`**). Pairs with `docs/WHATSAPP_TEMPLATE_REGISTRY.md` (the body/param contract the backend sends).
