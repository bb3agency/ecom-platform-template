...
- **Three buckets:** *common* → core (the shared **engine**: data/logic, checkout + payment, admin, ops, auth) · *configurable* → per client (the whole **storefront + account theme** — pages *and* presentation — plus design tokens & `FEATURE_*` flags) · *custom* → an extension folder the core ignores.
- **Each client pins `PLATFORM_VERSION`**; updating = replaying changelog entries to the latest tag.
- **Engine vs theme (since `backend-core 0.1.62`):** the customer-facing **storefront + account theme** — all `(storefront)` pages except `checkout`, all `app/(account)` pages, and their presentation components (`product/`, `layout/`, `storefront/`, `marketing/`, the cart-page components) — is **per-client and never synced**, so a produce shop and a sweets shop can differ in *layout*, not just words. The **shared engine** stays core: `lib/`/`stores/`/`hooks/`/`actions/`/`types/`, `components/ui`, **checkout + payment**, the admin & ops consoles, auth, root `layout.tsx`/`middleware`/`next.config`. **Hard rule: engine code must never import a theme file** — enforced by `frontend/scripts/check-theme-boundary.mjs` (in `ci:reliability-gates`). Design tokens still live in `globals.css`/`fonts`/`constants`/`content`/`public` (`merge=ours`); feature differences in `FEATURE_*` flags (default OFF); one-offs in the client extension layer (`backend/src/modules/client/**`, `frontend/app/(client)/**`, `frontend/components/client/**`).
- **CI enforces it:** `check-core-drift.sh` (no silent fork) + `check-token-contract.sh` (no broken theme) + `check-theme-boundary.mjs` (no engine→theme import) + compatibility check (no mismatched core pair).
...
| --- | --- | --- | --- |
| **Common** (all clients) | the **engine**: API client + `lib/`/`stores/`/`hooks/`/`actions/`/`types/`, `components/ui`, **checkout + payment**, admin & ops consoles, auth, root infra, all backend modules | **Core** (`core-manifest.json` → include) | `backend-core` / `frontend-core` semver |
| **Configurable** (per client) | the **storefront + account theme** (all `(storefront)` pages except `checkout`, all `app/(account)` pages, `product/`/`layout/`/`storefront/`/`marketing/` + cart-page components), palette/fonts (`app/globals.css`, `lib/fonts.ts`), brand identity (`lib/constants.ts`), **copy/content** (`lib/content.ts`), assets (`public/`), email content, `FEATURE_*` flags | Storefront theme + design layer + Ops/store config | Not core-versioned (orthogonal) |
...
- **All client identity AND copy are DATA in the design layer**, never hardcoded in core. Two per-client config files (both excluded from sync): **`lib/constants.ts`** = brand IDENTITY (name, logo, `STORAGE_PREFIX`, domains, contact); **`lib/content.ts`** = client COPY (taglines, blurbs, product-attribute defaults, homepage SEO). Palette → `globals.css` tokens; assets → `public/`; behaviour → `FEATURE_*` flags + Ops/env config. **Since `backend-core 0.1.62` the whole customer-facing storefront + account THEME is per-client too** — not just tokens/copy but the pages and presentation components themselves — so brands can differ in *layout and structure*, not only wording (a produce shop vs a sweets shop). The shared engine (`lib`/`stores`/`hooks`/`actions`/`types`, `components/ui`, checkout, admin, ops, auth) stays byte-identical and **must never import a theme file** (`check-theme-boundary.mjs`). Core components import identity + copy from `constants.ts`/`content.ts` and hardcode neither. **Rule: if a string in core reads like a brand name, location, or marketing line, it belongs in `constants.ts`/`content.ts` — or the file itself belongs in the per-client theme.**
...
- **Client identity AND copy never live in core code.** Brand name → `APP_NAME`; logo → `BRAND_LOGO_SRC`; storage keys → `STORAGE_PREFIX`; provider/contact → env or Ops config — all in **`lib/constants.ts`** (excluded). Marketing **copy** (taglines, blurbs, product-attribute defaults, homepage SEO) → **`lib/content.ts`** (excluded). The **entire storefront + account theme** (all `(storefront)` pages except `checkout`, all `app/(account)` pages, and the `product/`/`layout/`/`storefront/`/`marketing/` + cart-page components; plus email templates) is excluded from core in `core-manifest.json` — each client owns its own presentation. Because that theme is per-client, brand strings inside it are fine; `check-core-purity` only scans the shared engine. **Core TEST files and `backend/.env.example` are core (synced) since `0.1.29`** — a core change that touches a test or adds an env key reaches every client automatically; only client-extension tests (`src/modules/client/**/*.test.ts`, `frontend/{components/client,app/(client)}/**/*.test.*`) are excluded. (Secret env files stay untracked, never synced.)
