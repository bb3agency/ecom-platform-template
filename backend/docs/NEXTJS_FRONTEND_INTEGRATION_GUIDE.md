...
- **Admin order detail (`GET /admin/orders/:id`)** returns `packingBox` (`{ lengthCm, widthCm, heightCm, weightGrams, source, boxName }`) — the same dimensions used to rate the order — so the merchant sees the recommended carton to pack into. Render it on the order detail page (the admin storefront shows a "Packing box" card).
