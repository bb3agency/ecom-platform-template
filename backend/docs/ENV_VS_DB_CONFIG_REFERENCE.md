...
- **DB-layer override:** `StoreSettings.notifyEmailEnabled` / `notifySmsEnabled` / `notifyWhatsappEnabled` take precedence over the env flag when the `storeSettings` row exists. **Provider** credentials live in `/ops/config` (ops console); the **per-notification channel routing** is a merchant setting in Admin → Settings → Notifications (`GET`/`PATCH /api/v1/admin/settings/notifications`).
- **Per-template routing is MULTI-channel (backend-core 0.1.30):** `StoreSettings.primaryNotificationChannels[template]` is a SET of channels (`['EMAIL','WHATSAPP']`) — a notification is sent to EVERY selected + provisioned channel (on/off toggles in the admin panel). If none of the configured channels can currently deliver (e.g. WhatsApp turned off), order notifications fall back to **email**. WhatsApp/SMS only send to a recipient (customer, admin, merchant) who has a **phone number on file**. WhatsApp-for-OTP additionally requires the ops `OTP_WHATSAPP_ENABLED` gate.
